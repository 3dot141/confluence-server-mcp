    ...
    async process(request) {
        ...
        for (const diagram of sortedDiagrams) {
            try {
                ...
                const imageAttachment = await confluenceRepository.uploadAttachmentFromBase64(request.pageId, imageBuffer.toString('base64'), imageFileName, `Mermaid diagram ${diagram.id}`);
                const sourceFileName = `mermaid-diagram-${diagram.id}.mmd`;
                const sourceAttachment = await confluenceRepository.uploadAttachmentFromBase64(request.pageId, Buffer.from(diagram.source).toString('base64'), sourceFileName, `Source code for Mermaid diagram ${diagram.id}`);
...
//# sourceMappingURL=mermaid-publish.js.map