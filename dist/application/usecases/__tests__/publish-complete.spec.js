...
function setupRepositorySpies() {
    ...
    repositoryMock.uploadAttachmentFromBase64.mockResolvedValue({});
    repositoryMock.updatePage.mockImplementation(async (request) => ({
        id: request.pageId,
        title: request.title,
...
describe('PublishCompleteUseCase mermaid handling', () => {
    ...
    it('renders mermaid and replaces code block with uploaded attachment', async () => {
        ...
        expect(uploadAttachmentSpy).toHaveBeenCalledWith('1001', expect.any(String), 'mermaid-diagram-0.png');
        const updateRequest = updatePageSpy.mock.calls[0]?.[0];
    ...
    it('keeps distinct attachments for duplicate mermaid blocks', async () => {
        const { uploadAttachmentSpy, updatePageSpy } = setupRepositorySpies();
        vi.spyOn(MermaidRenderer.prototype, 'render').mockImplementation(async (source, options) => ({
            id: options.id,
            source,
            success: true,
            imageBuffer: PNG_BUFFER,
...
//# sourceMappingURL=publish-complete.spec.js.map